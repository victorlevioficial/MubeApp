...
///
/// Steps:
/// 1. Basic Info (Studio Name, Contact)
/// 2. Services Offered
/// 3. Address
class EnhancedOnboardingStudioFlow extends ConsumerStatefulWidget {
  final AppUser user;

  const EnhancedOnboardingStudioFlow({super.key, required this.user});

  @override
  ConsumerState<EnhancedOnboardingStudioFlow> createState() =>
      _EnhancedOnboardingStudioFlowState();
}

class _EnhancedOnboardingStudioFlowState
    extends ConsumerState<EnhancedOnboardingStudioFlow> {
  ...
  void initState() {
    ...
    _nomeEstudioController.text = formState.nome ?? widget.user.nome ?? '';
    ...
    _nomeEstudioController.addListener(
      () => ref
          ...
          .updateNome(_nomeEstudioController.text),
    );
  ...
  void _nextStep() {
    if (_currentStep == 1) {
      ...
      ref.read(onboardingFormProvider.notifier).updateServices(_selectedServices);
      setState(() => _currentStep++);
    }
  }
  ...
  void _finishOnboarding() {
    ...
    ref.read(onboardingControllerProvider.notifier).submitProfileForm(
          ...
          nome: _nomeEstudioController.text,
          location: location,
  ...
  Widget _buildStep2UI() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: AppSpacing.s8),
        Text(
          'Quais serviços seu estúdio oferece?',
          style: AppTypography.bodyMedium.copyWith(
            color: AppColors.textSecondary,
          ),
          textAlign: TextAlign.center,
        ...
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color:
                  _selectedServices.isEmpty ? AppColors.error : AppColors.border,
              width: 1,
            ),
          ),
          child: Column(
            ...
            children: [
              ...
              Text(
                _selectedServices.isEmpty
                    ...
                    : '${_selectedServices.length} serviço${_selectedServices.length > 1 ? 's' : ''} selecionado${_selectedServices.length > 1 ? 's' : ''}',
