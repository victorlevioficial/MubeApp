...
import 'flows/enhanced_onboarding_band_flow.dart';
import 'flows/enhanced_onboarding_contractor_flow.dart';
import 'flows/enhanced_onboarding_professional_flow.dart';
import 'flows/enhanced_onboarding_studio_flow.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return userAsync.when(
      ...
      data: (user) {
        ...
        // Delegate to Contractor Flow if applicable (Enhanced)
        if (user.tipoPerfil == AppUserType.contractor) {
          return EnhancedOnboardingContractorFlow(user: user);
        }

        // Delegate to Professional Flow if applicable (Enhanced)
        if (user.tipoPerfil == AppUserType.professional) {
          return EnhancedOnboardingProfessionalFlow(user: user);
        }

        // Delegate to Studio Flow if applicable (Enhanced)
        if (user.tipoPerfil == AppUserType.studio) {
          return EnhancedOnboardingStudioFlow(user: user);
        }

        // Delegate to Band Flow if applicable (Enhanced)
        if (user.tipoPerfil == AppUserType.band) {
          return EnhancedOnboardingBandFlow(user: user);
        }
