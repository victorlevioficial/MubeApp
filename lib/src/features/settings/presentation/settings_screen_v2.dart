...
/// New clean Settings screen — v2 redesign.
///
/// Design principles:
/// - iOS-style card groups: each section wrapped in a rounded surface card
/// - Monochrome icon containers: consistent neutral tones, no per-tile accent colors
/// - Value text inline on the right (e.g., current language)
/// - Indented dividers between items within a card
/// - Minimal, breathable layout with proper type hierarchy
///
/// The original [SettingsScreen] is preserved unchanged.
class SettingsScreenV2 extends ConsumerWidget {
  ...
  void _confirmDelete(BuildContext context, WidgetRef ref) async {
    ...
    final confirm = await AppOverlay.dialog<bool>(
      context: context,
      builder: (context) => const _DeleteAccountConfirmDialogV2(),
    );
...
class _DeleteAccountConfirmDialogV2 extends StatefulWidget {
  const _DeleteAccountConfirmDialogV2();

  @override
  State<_DeleteAccountConfirmDialogV2> createState() =>
      _DeleteAccountConfirmDialogV2State();
}

class _DeleteAccountConfirmDialogV2State
    extends State<_DeleteAccountConfirmDialogV2> {
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            // ── Value text (right side, e.g. language) ───────────────────
            if (valueText != null) ...[
