        ...
        when (call.method) {
            ...
            "compressVideo" -> {
                try {
                    ...
                    Log.i(
                        TAG,
                        "compressVideo requested path=$path quality=$quality deleteOrigin=$deleteOrigin startTime=$startTime duration=$duration includeAudio=$includeAudio frameRate=$frameRate"
                    )
                    ...
                    val dataSource = if (startTime != null || duration != null){
                        ...
                        Log.i(
                            TAG,
                            "compressVideo using TrimDataSource startUs=${(1000 * 1000 * (startTime ?: 0)).toLong()} durationUs=${(1000 * 1000 * (duration ?: 0)).toLong()} destPath=$destPath"
                        )
                        TrimDataSource(source, (1000 * 1000 * (startTime ?: 0)).toLong(), (1000 * 1000 * (duration ?: 0)).toLong())
                    }else{
                        Log.i(TAG, "compressVideo using UriDataSource without trim destPath=$destPath")
                    ...
                    transcodeFuture = Transcoder.into(destPath!!)
                            ...
                            .setListener(object : TranscoderListener {
                                ...
                                override fun onTranscodeCompleted(successCode: Int) {
                                    Log.i(TAG, "onTranscodeCompleted successCode=$successCode destPath=$destPath")
                                ...
                                override fun onTranscodeCanceled() {
                                    Log.w(TAG, "onTranscodeCanceled path=$path destPath=$destPath")
                                    result.success(null)
                                }
                                ...
                                override fun onTranscodeFailed(exception: Throwable) {
                                    Log.e(
                                        TAG,
                                        "onTranscodeFailed path=$path destPath=$destPath startTime=$startTime duration=$duration quality=$quality includeAudio=$includeAudio frameRate=$frameRate error=${exception.message}",
                                        exception
                                    )
                                    result.success(null)
                                }
                    ...
                    Log.e(TAG, "compressVideo failed before transcoder started", exception)
                    result.success(null)
                }
            }
            else -> {
